...
@Author : TMG HITSZ
@Date   : 2023/10/28
@Desc   : ablation without peer solutions during answer revision
=================================================
'''
        ...
        for round_num in range(args.rounds):
            ...
            for j, agent_context in enumerate(agent_contexts):
                if round_num == 0: # ROUND 0: generate initial answer
                    ...
                    # Solution ablation (Section 4.4): remove the complete
                    # Stage-2 dialogue before revision. Keep only this agent's
                    # original question and initial answer, then add feedback.
                    agent_context = agent_context[:2]
                    agent_contexts[j] = agent_context
                    ...
                    if args.task in ['GSM8K', 'SVAMP', 'AddSub', 'SingleEq', 'MultiArith']:  # number
                        content += f"Using other agents' feedbacks as additional information, " \
                                   f"can you provide your answer to the problem? \n " \
                        ...
                        content += f"Using other agents' feedbacks as additional information, " \
                                   f"can you provide your answer to the problem? Examine your solution and other agents' feedback step by step. " \
...
def log_param(args):
    args_str = f'\n--------- peer review w/o solution parameters ----------\n'
...
if __name__ == "__main__":
    # 1. args
    args = params.ablation_solution_args()
    log_param(args)
